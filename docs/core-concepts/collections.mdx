...
db = vxdb.Database(path="./my_data")  # persistent
...
<Tip>
When using a persistent database (`path=`), collections and all their data are automatically saved to disk. Reopen the same path to restore everything.
</Tip>
...
<Tip>
Upserting with an existing ID replaces that entry entirely — vector, metadata, and document.
</Tip>
